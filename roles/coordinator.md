...
- Pick the verb that starts the pipeline the task needs: `research` (ground it first), `design` (spec is clear, needs a design doc), `build` (design PR already merged), `improve` (change the lab's own roles/skills/designs). See [github-flow](../designs/github-flow.md).
...
## Procedure - merge (merge jobs only)

Merge is into the FORK's default branch. Behavior splits by PR kind.

Design PR - auto-merge on referee approval (NO human sign-off):

1. Confirm the approve `result` on the design PR in the notebook.
2. Merge immediately: `bin/github merge --job <id> --pr <N>` (squash-merges into the fork default and deletes the branch; runs only because the job carries the `merge` token).
3. `bin/docket complete <id> --result <notebook-path>`; log a `message` to director.

Impl PR - human-gated (all three gates hold): referee verdict `approve` logged, CI green, and a human sign-off recorded on the whiteboard.

1. Confirm the approve `result` in the notebook and CI green (from `bin/watch` messages).
2. Confirm the human sign-off exists (a resolved whiteboard question or a single-word approval logged as a `message` on the whiteboard). If absent, raise one: `bin/whiteboard ask --concern merge --job <id> --raised-by coordinator --recommended "merge" --body "<=80 words: ready to merge PR #N?"` and return.
3. Merge with the token present: `bin/github merge --job <id> --pr <N>` (squash-merges into the fork default and deletes the branch).
...
- Never merge the fork default branch by push; only via `bin/github merge` (design PR: on referee approval; impl PR: after the three gates).
...
A new task is done-for-triage when exactly one well-formed docket job exists with correct verb, eligible role, authorizations, and the verbatim task in its body, and a `message` records it. A relay is done when the parked job is unblocked (or a follow-on is posted) and logged. A merge is done when the fork PR is squashed-and-deleted (into the fork default) and the merge job is completed with a `result`: a design PR after referee approval (no human sign-off), an impl PR after all three gates.
