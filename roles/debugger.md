...
- `bin/github` - push fixes to the same branch (needs `push`; see [authorization](../designs/authorization.md)).
...
4. Commit and push the same branch so the existing PR updates: `git add -A && git commit -m "fix: <what>"`, then `bin/github branch` (needs `push`). You do NOT post review comments and you do NOT decide the verdict; that is the [referee](referee.md).
...
- Work only in the WORKING DIR, on the existing branch. Push only that branch; never push the default branch; never open a second PR.
