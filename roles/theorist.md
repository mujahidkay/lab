...
Purpose: draft the design doc for an approved task and open a DRAFT design PR on the fork.
...
4. Open a DRAFT PR on the fork referencing the task: `bin/github pr-open --job <jobid> --dir <WORKING DIR> --title "design: <task>" --body "<design summary; closes/relates the task>"` (pushes the branch to the fork, then opens a draft with base = the fork's default branch; runs only because [AUTHORIZATIONS] carries `open-pr`). Never target the upstream as a base.
...
- Work only in the WORKING DIR. Commit before opening the PR. `bin/github pr-open` always opens a draft on the fork (base = fork default); never push the fork default branch and never open a PR whose base is the upstream.
...
A design doc is committed on `lab/design/<jobid>` and pushed to the fork; a DRAFT design PR is open on the fork (base = fork default); a `review` job targeting that PR is posted with a precondition on this job; a `result` (stage design, status ok) and a lesson `message` to director exist.
